...
- **L2 — Deterministic auditor:** consumes the L1 IR and emits findings with source evidence and epistemic status (CONFIRMED / CANDIDATE / OBSERVATION), seals an AuditArtifact (`crucible-audit/v1`), and documents 4 abstained checks as explicit limitations rather than silently passing them. 9 checks are emitted: BROKEN_REFERENCE, SELF_COMPOSITION, COMPOSITION_CYCLE, ORPHAN_SKILL, REQUIREMENT_WITHOUT_CHECK, STRUCTURAL_REDUNDANCY, METHODOLOGICAL_VACUITY (rules but no steps and no checks — detects skills that say what to do but never how), NORMATIVE_CONFLICT (same subject, opposite modality — detects contradictions within and across skills), and SEMANTIC_REDUNDANCY (Jaccard token overlap >= 2/3 with Fraction, no floats — detects skills that cover the same ground; LLM confirmation layer deferred). The IR extracts rule subjects and procedural steps to support these checks.
