...
| L2 | Deterministic single-corpus auditor | **Implemented:** the auditor consumes the L1 IR, emits findings with source evidence and epistemic status (CONFIRMED / CANDIDATE / OBSERVATION), seals an AuditArtifact (`crucible-audit/v1`), and documents 0 abstained checks (all 14 are emitted). 14 checks are emitted: BROKEN_REFERENCE, SELF_COMPOSITION, COMPOSITION_CYCLE, ORPHAN_SKILL, REQUIREMENT_WITHOUT_CHECK, STRUCTURAL_REDUNDANCY, METHODOLOGICAL_VACUITY, NORMATIVE_CONFLICT, SEMANTIC_REDUNDANCY (Jaccard with Fraction, no floats; LLM layer deferred), CONDITIONAL_CONTRADICTION (pattern-based condition extraction with effective polarity computation), SCOPE_TRIGGER_MISMATCH (trigger extraction from description, zero-overlap to rule tokens; 2 CANDIDATE on real corpus), DESCRIPTION_BODY_GAP (substantive description with zero extractable body structure; 27 CANDIDATE on real corpus, 28%), CHECK_WITHOUT_ORACLE (oracle_kind extraction: question/checkbox/command/unknown; 16 CANDIDATE on real corpus, 9% of checks), and CLAIM_WITHOUT_PROVENANCE (claim extraction: percentage/time/count/standard/year with provenance detection; 0 findings on real corpus). The IR extracts rule subjects, conditions, triggers, procedural steps, check oracle_kind, and rule claims. The modality regex correctly captures "MUST NOT" as MUST_NOT. Cross-process digest verified identical. |
| L2.5 | Semantic redundancy confirmation layer | **Implemented:** the confirmation layer (`confirm.py`) takes SEMANTIC_REDUNDANCY CANDIDATEs from the L2 audit and asks an executor (Nemotron via Nebius, or deterministic mock) whether each pair is semantically redundant. The confirmation is a separate artifact (`crucible-confirmation/v1`) with its own SHA-256 digest. The L2 audit artifact is NEVER modified — the confirmation is an OBSERVATION, not a promotion to CONFIRMED. If NEBIUS_API_KEY is not set, the confirmation is BLOCKED, not simulated. The mock executor allows testing without external dependencies. Real corpus: 0 confirmations (0 SEMANTIC_REDUNDANCY CANDIDATEs). Cross-process determinism confirmed. |
