...
| L2 | Deterministic single-corpus auditor | **Implemented:** the auditor consumes the L1 IR, emits findings with source evidence and epistemic status (CONFIRMED / CANDIDATE / OBSERVATION), seals an AuditArtifact (`crucible-audit/v1`), and documents 5 abstained checks as explicit limitations. 8 checks are emitted: BROKEN_REFERENCE, SELF_COMPOSITION, COMPOSITION_CYCLE, ORPHAN_SKILL, REQUIREMENT_WITHOUT_CHECK, STRUCTURAL_REDUNDANCY, METHODOLOGICAL_VACUITY (rules but no steps and no checks), and NORMATIVE_CONFLICT (same subject, opposite modality). The IR now extracts rule subjects and procedural steps. The modality regex was fixed to correctly capture "MUST NOT" as MUST_NOT. Cross-process digest verified identical. |
