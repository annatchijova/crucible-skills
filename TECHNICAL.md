...
| L2 | Deterministic single-corpus auditor | **Implemented:** the auditor consumes the L1 IR, emits findings with source evidence and epistemic status (CONFIRMED / CANDIDATE / OBSERVATION), seals an AuditArtifact (`crucible-audit/v1`), and documents 4 abstained checks as explicit limitations. 9 checks are emitted: BROKEN_REFERENCE, SELF_COMPOSITION, COMPOSITION_CYCLE, ORPHAN_SKILL, REQUIREMENT_WITHOUT_CHECK, STRUCTURAL_REDUNDANCY, METHODOLOGICAL_VACUITY (rules but no steps and no checks), NORMATIVE_CONFLICT (same subject, opposite modality), and SEMANTIC_REDUNDANCY (Jaccard token overlap >= 2/3 with Fraction, no floats; LLM confirmation layer deferred). The IR extracts rule subjects and procedural steps. The modality regex correctly captures "MUST NOT" as MUST_NOT. Cross-process digest verified identical. |
