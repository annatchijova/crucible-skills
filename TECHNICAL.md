...
| L2 | Deterministic single-corpus auditor | **Implemented:** the auditor consumes the L1 IR, emits findings with source evidence and epistemic status (CONFIRMED / CANDIDATE / OBSERVATION), seals an AuditArtifact (`crucible-audit/v1`), and documents 2 abstained checks as explicit limitations. 12 checks are emitted: BROKEN_REFERENCE, SELF_COMPOSITION, COMPOSITION_CYCLE, ORPHAN_SKILL, REQUIREMENT_WITHOUT_CHECK, STRUCTURAL_REDUNDANCY, METHODOLOGICAL_VACUITY, NORMATIVE_CONFLICT, SEMANTIC_REDUNDANCY (Jaccard with Fraction, no floats; LLM layer deferred), CONDITIONAL_CONTRADICTION (pattern-based condition extraction with effective polarity computation), SCOPE_TRIGGER_MISMATCH (trigger extraction from description, zero-overlap to rule tokens; 2 CANDIDATE on real corpus), and DESCRIPTION_BODY_GAP (substantive description with zero extractable body structure; 27 CANDIDATE on real corpus, 28%). The IR extracts rule subjects, conditions, triggers, and procedural steps. The modality regex correctly captures "MUST NOT" as MUST_NOT. Cross-process digest verified identical. |
