...
| L2 | Deterministic single-corpus auditor | **Implemented:** the auditor consumes the L1 IR, emits findings with source evidence and epistemic status (CONFIRMED / CANDIDATE / OBSERVATION), seals an AuditArtifact (`crucible-audit/v1`), and documents 3 abstained checks as explicit limitations. 10 checks are emitted: BROKEN_REFERENCE, SELF_COMPOSITION, COMPOSITION_CYCLE, ORPHAN_SKILL, REQUIREMENT_WITHOUT_CHECK, STRUCTURAL_REDUNDANCY, METHODOLOGICAL_VACUITY, NORMATIVE_CONFLICT, SEMANTIC_REDUNDANCY (Jaccard with Fraction, no floats; LLM layer deferred), and CONDITIONAL_CONTRADICTION (pattern-based condition extraction with effective polarity computation). The IR extracts rule subjects, conditions, and procedural steps. The modality regex correctly captures "MUST NOT" as MUST_NOT. Cross-process digest verified identical. |
